...
// ComputeTypesPass used to take the variable values reaching a loop's condition and a loop's exit
// from the fall-through path only, discarding the values coming from `continue` and `break`.
// A write performed on such a path stayed invisible, so the variable was narrowed down to the
// remaining writes only, and reading it produced an unsafe downcast to a final class.
